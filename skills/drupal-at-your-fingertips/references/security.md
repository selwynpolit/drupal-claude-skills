...
**Source**: [Drupal at Your Fingertips - security](https://drupalatyourfingertips.com/security)
**Author**: Selwyn Polit
...
Robot webcrawlers (Bing, Yandex, Semrush, etc.) can attempt to crawl a Drupal View's search results pages, following links to each of the view's filtering options, placing extra load on the site without necessarily improving search visibility.
...
Alternatively, block or reroute known crawler user agents in `.htaccess` (edit the query-string condition and user-agent list for your use case):

```
RewriteCond %{QUERY_STRING} .
RewriteCond %{HTTP_USER_AGENT} "AdsBot-Google|Ahrefs|Amazonbot|Applebot|Baiduspider|bingbot|BLEXBot|Bytespider|CCBot|DotBot|MJ12bot|PetalBot|SemrushBot|YandexBot" [NC]
RewriteRule ^.* - [F,L]
```
...
A small module can let an admin tag a node with a term, and a user with the matching term on their own account can edit that node even without normal Drupal edit permissions.

This requires:
1. A vocabulary, e.g. `wecc_edit_access`
2. A term-reference field on the node, e.g. `field_wecc_edit_access`, pointing to that vocabulary
3. A term-reference field on the user, e.g. `field_wecc_user_edit_access`, pointing to the same vocabulary

In `web/modules/custom/wecc_node_access/wecc_node_access.module`:
...
function wecc_node_access_node_access(NodeInterface $node, $op, AccountInterface $account): AccessResult {
  ...
  if ($op == 'update') {
    ...
    $user_tids = array_column($user_edit_access_tids, 'target_id');
 ...
 *
 * Hide field_wecc_edit_access from users without the administrator role,
 * so they can't grant themselves edit access.
 */
function wecc_node_access_form_alter(&$form, FormStateInterface $form_state, $form_id) {
  $user = \Drupal::currentUser();
  if (!$user->hasRole('administrator') && isset($form['field_wecc_edit_access'])) {
    $form['field_wecc_edit_access']['#access'] = FALSE;
  }
}
```

This handles caching correctly, so changes to the term on the node or the user are reflected immediately.

A role-based variant is also possible: add a role-reference field (e.g. `field_role_access`) to the node instead of a taxonomy term, and check `array_intersect($node_edit_access_roles, $user->getRoles())` in `hook_node_access()` instead of comparing term IDs.

## File and directory permissions

_Merged from the d9book "general" chapter, which was too large to keep as a single reference file._

Typical deployment step to restore file and directory permissions for a Drupal site (adjust the path for your web root):
...
`chmod 755` on directories gives the owner read/write/execute and group/others read/execute, so directories can be traversed by the web server. `chmod 644` on files gives the owner read/write and group/others read-only, letting the web server read files while restricting modification to the owner. `-exec ... {} +` batches the `chmod` calls for efficiency instead of invoking it once per file.
