...
**Source**: [Drupal at Your Fingertips - drush](https://drupalatyourfingertips.com/drush)
**Author**: Selwyn Polit
...
Drush commands need a single file in `module/src/Drush/Commands` which has the class (extending `DrushCommands`) and the annotated function which executes the command.
...
namespace Drupal\drush_play2\Drush\Commands;
 ...
 * A Drush commandfile.
 */
final class DrushPlay2Commands extends DrushCommands {

  /**
   * Constructs a DrushPlay2Commands object.
   */
...
Here is a command that finds all uses of the button paragraph type.  It is in the `web/modules/custom/drush_play/src/Drush/Commands/DrushPlayCommands.php` file:
...
final class DrushPlayCommands extends DrushCommands {
  ...
  /**
   * Find content types that use the "button" paragraph type.
   */
...
The command `drush drush_play:find-button-usage` will output a list of content types that use the button paragraph type. e.g.

```
$ drush fbu
  content_type: button_tester
  field_name: field_buttons
```

For a little more flexibility, this function can be added to the above file to list the content types, as well as the most recent 5 nodes of each type that use the paragraph type.  This is in the `web/modules/custom/drush_play/src/Drush/Commands/DrushPlayCommands.php` file:
...
The command's output looks like this when searching for a paragraph type named "button":
